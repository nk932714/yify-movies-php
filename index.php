...
<html xmlns="http://www.w3.org/1999/xhtml">
 <head>
  ...
  <?php
        ...
        if (isset($_GET['queryTerm'])) {  $queryTerm = $_GET["queryTerm"]; $queryTerm = mb_strtolower($queryTerm); /*convert upper case to lower case*/ $queryTerm = urlencode($queryTerm); }   else { $queryTerm = "";	}
        
        
        
        // main code starts below
          $yts = new YTS();
          $movies = $yts->listMovies($quality, 20, $page1, $queryTerm, $rating, $genre, $sort_by); // All quality, limit etc
              ...
              class YTS
              	{
                	...
                	function listMovies(
	                      ...
	                      $queryTerm = 0,                             // $queryTerm Used for movie search, matching on: Movie Title/IMDb Code, Actor Name/IMDb Code, Director Name/IMDb Code
	                            ...
	                            )
		{
		              ...
		              $parameters = '?limit=' . $limit . '&page=' . $page . '&quality=' . $quality . '&minimum_rating=' . $minimum_rating . '&queryTerm=' . $queryTerm . '&genre=' . $genre . '&sort_by=' . $sort_by . '&order_by=' . $order_by . '&with_rt_ratings=' . $with_rt_ratings;
 ...
 <body>
       <div id="rounded">
	      ...
	      <a href="javascript:" id="return-to-top"><i> &#x25b2;</i></a>    <!-- Return to Top -->
			  ...
			  } } else { echo '<br><br>NO matching result found<br><br>Either movie name is wrong or movie is not on yify torrents database yet<br><br>Click to search on <a href="https://duckduckgo.com/?q='.$queryTerm.' torrent link">DuckDuckGO</a>&emsp;&emsp;<a href="https://www.google.com/search?q='.$queryTerm.' torrent link">Google</a>&emsp;&emsp;<a href="https://search.yahoo.com/search?p='.$queryTerm.' torrent link">Yahoo!</a>&emsp;&emsp; <br><br>' ; }
                  ?>
                   <le><a href="<?php echo $full_url1; ?>">Reload</a></le><le> <le><a href="<?php $page2 = $page1 + 1; $next_page = $full_url1.'?page='.$page2."&quality=".$quality."&genre=".$genre."&rating=".$rating."&sort_by=".$sort_by."&queryTerm=".$queryTerm;  echo $next_page ?>">Next Page</a></le><le><le><a href="<?php $page3 = $page1 - 1; $prev_page = $full_url1.'?page='.$page3."&quality=".$quality."&genre=".$genre."&rating=".$rating."&sort_by=".$sort_by."&queryTerm=".$queryTerm; echo $prev_page ?>">Previous Page</a></le><le>
        </div>
  <div class="clear"></div>
